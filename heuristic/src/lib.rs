...
mod model {
    ...
    extern "C" {
        ...
        pub fn model(
            ...
            sources: *const u32,
            targets: *const u32,
            node_batch: *const u32,
            edge_batch: *const u32,
            results: *mut f32,
        );
    }
}
...
pub fn model(batch: &Graph, result_buf: &mut [f32]) {
    ...
    let node_batch = batch.node_batch.as_ptr();
    let edge_batch = batch.edge_batch.as_ptr();
    ...
    unsafe {
        model::model(
            num_nodes, num_edges, num_graphs, nodes, sources, targets,
            node_batch, edge_batch, results,
        )
    };
}
