...
impl Statistics {
    ...
    pub(crate) fn increment_discarded_tableaux(&self) -> usize {
        self.discarded_tableaux.fetch_add(1, Relaxed)
    }

    pub(crate) fn increment_enqueued_tableaux(&self) -> usize {
        self.enqueued_tableaux.fetch_add(1, Relaxed)
    }

    pub(crate) fn increment_expanded_tableaux(&self) -> usize {
        self.expanded_tableaux.fetch_add(1, Relaxed)
    }

    pub(crate) fn exhausted_tableaux(&self, exhausted: u16) -> usize {
        self.exhausted_tableaux
            .fetch_add(exhausted as usize, Relaxed)
    }

    pub(crate) fn increment_total_tableaux(&self) -> usize {
